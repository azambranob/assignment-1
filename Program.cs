       ...
       */
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            try
            {
                //declare new variable for the sum and the iteration
                var sum = 0;
                var i = 0;
                // while loop to ad the primary diagonal sum and secondary
                while (i < bulls_grid.Length)
                {
                    sum += bulls_grid[i,i];
                    sum += bulls_grid[i,bulls_grid.Length - 1 - i];
                    i++;
                }

                // if the array his an odd number the get rid of the number in the middle add it twice
                if (bulls_grid.Length % 2 != 0)
                {
                    int j = bulls_grid.Length / 2;
                    sum -= bulls_grid[j,j];
                }
